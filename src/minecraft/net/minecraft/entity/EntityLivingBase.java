...
public abstract class EntityLivingBase extends Entity {
	...
	public static final int EaZy = 1117;
	...
	// TODO: Making visibletime4hypixel antibots
	public int ticksVisible = 0;
	...
	// private static final String __OBFID = "http://https://fuckuskid00001549";
	...
	protected void func_180433_a(final double p_180433_1_, final boolean p_180433_3_, final Block p_180433_4_,
		...
		if (!worldObj.isRemote && fallDistance > 3.0F && p_180433_3_) {
			final IBlockState var6 = worldObj.getBlockState(p_180433_5_);
			final Block var7 = var6.getBlock();
			final float var8 = MathHelper.ceiling_float_int(fallDistance - 3.0F);
			...
			if (var7.getMaterial() != Material.air) {
				...
				final int var11 = (int) (150.0D * var9);
				((WorldServer) worldObj).func_175739_a(EnumParticleTypes.BLOCK_DUST, posX, posY, posZ, var11, 0.0D,
						0.0D, 0.0D, 0.15000000596046448D, new int[] { Block.getStateId(var6) });
			}
		}
	...
	public void onEntityUpdate() {
		...
		if (isEntityAlive()) {
			if (isEntityInsideOpaqueBlock()) {
				...
				final double var2 = worldObj.getWorldBorder().getClosestDistance(this)
						+ worldObj.getWorldBorder().getDamageBuffer();

				if (var2 < 0.0D) {
					attackEntityFrom(DamageSource.inWall,
							Math.max(1, MathHelper.floor_double(-var2 * worldObj.getWorldBorder().func_177727_n())));
				}
			}
		}
		...
		final boolean var7 = var1 && ((EntityPlayer) this).capabilities.disableDamage;
			...
			if (!canBreatheUnderwater() && !this.isPotionActive(Potion.waterBreathing.id) && !var7) {
				...
				if (getAir() == -20) {
					...
					for (int var3 = 0; var3 < 8; ++var3) {
						...
						final float var6 = rand.nextFloat() - rand.nextFloat();
						worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, posX + var4, posY + var5, posZ + var6,
		...
		if (deathTime == 20) {
			...
			for (var1 = 0; var1 < 20; ++var1) {
				...
				worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL,
						posX + rand.nextFloat() * width * 2.0F - width, posY + rand.nextFloat() * height,
						posZ + rand.nextFloat() * width * 2.0F - width, var8, var4, var6, new int[0]);
			}
		}
	}
	...
	protected void updatePotionEffects() {
		...
		final int var11 = dataWatcher.getWatchableObjectInt(7);
		...
		if (var11 > 0) {
			...
			if (var4 && var11 > 0) {
				...
				final double var9 = (var11 & 255) / 255.0D;
				worldObj.spawnParticle(var12 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB,
						posX + (rand.nextDouble() - 0.5D) * width, posY + rand.nextDouble() * height,
						posZ + (rand.nextDouble() - 0.5D) * width, var5, var7, var9, new int[0]);
			}
		}
	}
	...
	protected void func_175135_B() {
		if (activePotionsMap.isEmpty()) {
			...
			final int var1 = PotionHelper.calcPotionLiquidColor(activePotionsMap.values());
			dataWatcher.updateObject(8, (byte) (PotionHelper.func_82817_b(activePotionsMap.values()) ? 1 : 0));
			dataWatcher.updateObject(7, var1);
	...
	public void addPotionEffect(final PotionEffect p_70690_1_) {
		if (isPotionApplicable(p_70690_1_)) {
			if (activePotionsMap.containsKey(p_70690_1_.getPotionID())) {
				...
				onChangedPotionEffect((PotionEffect) activePotionsMap.get(p_70690_1_.getPotionID()), true);
			} else {
	...
	public void removePotionEffect(final int p_82170_1_) {
		final PotionEffect var2 = (PotionEffect) activePotionsMap.remove(p_82170_1_);
	...
	public void setHealth(final float p_70606_1_) {
		dataWatcher.updateObject(6, MathHelper.clamp_float(p_70606_1_, 0.0F, getMaxHealth()));
	}
			...
			if (getHealth() <= 0.0F) {
				...
				if (entity != null) {
					...
					if (entity instanceof EntityPlayer) {
						...
						final EntityWolf var5 = (EntityWolf) entity;
				...
				if (var3) {
					...
					if (entity != null) {
						double kbX = entity.posX - posX;
						double kbZ = entity.posZ - posZ;

						for (; kbX * kbX + kbZ * kbZ < 1.0E-4D;) {
							kbX = (Math.random() - Math.random()) * 0.01D;
							kbZ = (Math.random() - Math.random()) * 0.01D;
						}

						attackedAtYaw = (float) (Math.atan2(kbZ, kbX) * 180.0D / Math.PI - rotationYaw);
						knockBack(entity, amount, kbX, kbZ);
					} else {
	...
	public void renderBrokenItemStack(final ItemStack p_70669_1_) {
		...
		for (int var2 = 0; var2 < 5; ++var2) {
			...
			final double var4 = -rand.nextFloat() * 0.6D - 0.3D;
			...
			worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, var6.xCoord, var6.yCoord, var6.zCoord, var3.xCoord,
					var3.yCoord + 0.05D, var3.zCoord, new int[] { Item.getIdFromItem(p_70669_1_.getItem()) });
		}
	}
	 ...
	 */
	public void knockBack(final Entity p_70653_1_, final float p_70653_2_, final double p_70653_3_,
			final double p_70653_5_) {
	...
	public boolean isOnLadder() {
		...
		final Block var4 = worldObj.getBlockState(new BlockPos(var1, var2, var3)).getBlock();
		return (var4 == Blocks.ladder || var4 == Blocks.vine)
				&& (!(this instanceof EntityPlayer) || !((EntityPlayer) this).isSpectatorMode());
	}
	...
	public void fall(final float distance, final float damageMultiplier) {
		...
		final PotionEffect var3 = getActivePotionEffect(Potion.jump);
		...
		if (var5 > 0) {
			...
			final int var6 = MathHelper.floor_double(posX);
			final int var7 = MathHelper.floor_double(posY - 0.20000000298023224D);
			final int var8 = MathHelper.floor_double(posZ);
			final Block var9 = worldObj.getBlockState(new BlockPos(var6, var7, var8)).getBlock();
	...
	public int getTotalArmorValue() {
		int var1 = 0;
		final ItemStack[] var2 = getInventory();
	...
	public final void setArrowCountInEntity(final int p_85034_1_) {
		dataWatcher.updateObject(9, (byte) p_85034_1_);
	}
	...
	public void swingItem() {
		if (!isSwingInProgress || swingProgressInt >= getArmSwingAnimationEnd() / 2 || swingProgressInt < 0) {
			...
			if (worldObj instanceof WorldServer) {
				((WorldServer) worldObj).getEntityTracker().sendToAllTrackingEntity(this,
		...
		for (int var10 = -var9; var10 <= var9; ++var10) {
			for (int var11 = -var9; var11 < var9; ++var11) {
				if (var10 != 0 || var11 != 0) {
					...
					final AxisAlignedBB var2 = getEntityBoundingBox().offset(var10, 1.0D, var11);
					...
					if (worldObj.func_147461_a(var2).isEmpty()) {
						...
						if (World.doesBlockHaveSolidTopSurface(worldObj, new BlockPos(var12, (int) posY - 1, var13))
								|| worldObj.getBlockState(new BlockPos(var12, (int) posY - 1, var13)).getBlock()
	...
	public void moveEntityWithHeading(final float strafe, final float forward) {
		...
		if (isServerWorld()) {
			...
			if (isInWater() && (!(this instanceof EntityPlayer) || !((EntityPlayer) this).capabilities.isFlying)) {
				...
				moveFlying(strafe, forward, var6);
				...
				if (isCollidedHorizontally
						&& isOffsetPositionInLiquid(motionX, motionY + 0.6000000238418579D - posY + var8, motionZ)) {
				...
				moveFlying(strafe, forward, var5);
				var3 = 0.91F;
				...
				if (isOnLadder()) {
					...
					final boolean var7 = isSneaking() && this instanceof EntityPlayer;
		...
		if (!worldObj.isRemote) {
			...
			for (int var2 = 0; var2 < 5; ++var2) {
				...
				if (!ItemStack.areItemStacksEqual(var4, var3)) {
					((WorldServer) worldObj).getEntityTracker().sendToAllTrackingEntity(this,
							new S04PacketEntityEquipment(getEntityId(), var2, var4));
	...
	protected float func_110146_f(final float p_110146_1_, float p_110146_2_) {
		final float var3 = MathHelper.wrapAngleTo180_float(p_110146_1_ - renderYawOffset);
	...
	public void onLivingUpdate() {
		...
		if (newPosRotationIncrements > 0) {
			final double var1 = posX + (newPosX - posX) / newPosRotationIncrements;
			final double var3 = posY + (newPosY - posY) / newPosRotationIncrements;
			final double var5 = posZ + (newPosZ - posZ) / newPosRotationIncrements;
			final double var7 = MathHelper.wrapAngleTo180_double(newRotationYaw - rotationYaw);
	...
	public void onItemPickup(final Entity p_71001_1_, final int p_71001_2_) {
		if (!p_71001_1_.isDead && !worldObj.isRemote) {
			final EntityTracker var3 = ((WorldServer) worldObj).getEntityTracker();
	...
	public boolean canEntityBeSeen(final Entity p_70685_1_) {
		return worldObj.rayTraceBlocks(new Vec3(posX, posY + getEyeHeight(), posZ),
				new Vec3(p_70685_1_.posX, p_70685_1_.posY + p_70685_1_.getEyeHeight(), p_70685_1_.posZ)) == null;
	}

	/**
	 * returns true if the block provided in the argument can be seen. (Raytrace)
	 */
	public boolean canBlockBeSeen(final BlockPos p_70685_1_) {
		return worldObj.rayTraceBlocks(new Vec3(posX, posY + getEyeHeight(), posZ),
				new Vec3(p_70685_1_.getX(), p_70685_1_.getY(), p_70685_1_.getZ())) == null;
	}
	...
	public Vec3 getLook(final float p_70676_1_) {
		if (p_70676_1_ == 1.0F) {
			...
			final float var3 = prevRotationYawHead + (rotationYawHead - prevRotationYawHead) * p_70676_1_;
