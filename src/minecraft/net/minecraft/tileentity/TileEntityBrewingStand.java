...
public static final int EaZy = 1571;
	...
	// private static final String __OBFID = "http://https://fuckuskid00000345";
	...
	public void update() {
		...
		if (!worldObj.isRemote) {
			final boolean[] var1 = func_174902_m();
			...
			if (!var1.getItem().isPotionIngredient(var1)) {
				...
				for (int var3 = 0; var3 < 3; ++var3) {
					if (brewingItemStacks[var3] != null && brewingItemStacks[var3].getItem() == Items.potionitem) {
						final int var4 = brewingItemStacks[var3].getMetadata();
		...
		if (canBrew()) {
			...
			for (int var2 = 0; var2 < 3; ++var2) {
				if (brewingItemStacks[var2] != null && brewingItemStacks[var2].getItem() == Items.potionitem) {
					final int var3 = brewingItemStacks[var2].getMetadata();
	...
	public ItemStack decrStackSize(final int index, final int count) {
		if (index >= 0 && index < brewingItemStacks.length) {
			final ItemStack var3 = brewingItemStacks[index];
	...
	public ItemStack getStackInSlotOnClosing(final int index) {
		if (index >= 0 && index < brewingItemStacks.length) {
			final ItemStack var2 = brewingItemStacks[index];
			brewingItemStacks[index] = null;
			return var2;
		} else {
			return null;
