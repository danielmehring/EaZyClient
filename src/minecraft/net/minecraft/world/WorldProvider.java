...
public static final int EaZy = 1855;
	...
	// private static final String __OBFID = "http://https://fuckuskid00000386";
	...
	protected void registerWorldChunkManager() {
		final WorldType var1 = worldObj.getWorldInfo().getTerrainType();

		if (var1 == WorldType.FLAT) {
			final FlatGeneratorInfo var2 = FlatGeneratorInfo
					.createFlatGeneratorFromString(worldObj.getWorldInfo().getGeneratorOptions());
	...
	public IChunkProvider createChunkGenerator() {
		return terrainType == WorldType.FLAT
				? new ChunkProviderFlat(worldObj, worldObj.getSeed(), worldObj.getWorldInfo().isMapFeaturesEnabled(),
						generatorSettings)
				: terrainType == WorldType.DEBUG_WORLD ? new ChunkProviderDebug(worldObj)
						: terrainType == WorldType.CUSTOMIZED
								? new ChunkProviderGenerate(worldObj, worldObj.getSeed(),
										worldObj.getWorldInfo().isMapFeaturesEnabled(), generatorSettings)
								: new ChunkProviderGenerate(worldObj, worldObj.getSeed(),
										worldObj.getWorldInfo().isMapFeaturesEnabled(), generatorSettings);
	}
