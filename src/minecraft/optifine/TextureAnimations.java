...
public static final int EaZy = 1968;
	...
	public static TextureAnimation[] getTextureAnimations(final IResourcePack[] rps) {
		...
		final TextureAnimation[] var5 = (TextureAnimation[]) list.toArray(new TextureAnimation[list.size()]);
		return var5;
	}

	public static TextureAnimation[] getTextureAnimations(final IResourcePack rp) {
		final String[] animPropNames = ResUtils.collectFiles(rp, "mcpatcher/anim", ".properties", (String[]) null);
		...
		if (animPropNames.length <= 0) {
			...
			final TextureAnimation[] var12 = (TextureAnimation[]) list.toArray(new TextureAnimation[list.size()]);
			return var12;
		}
	}
			...
			if (x >= 0 && y >= 0 && width >= 0 && height >= 0) {
				...
				if (imageBytes == null) {
					...
					if (!Config.hasResource(locTexTo)) {
						...
						final TextureAnimation anim = new TextureAnimation(texFrom, imageBytes, texTo, locTexTo, x, y,
