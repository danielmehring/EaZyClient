...
public static final int EaZy = 1946;
	...
	public static PlayerItemModel parseItemModel(final JsonObject obj) {
		...
		if (!Config.equals(type, "PlayerItem")) {
			...
			for (int modelRenderers = 0; modelRenderers < models.size(); ++modelRenderers) {
				...
				if (baseId != null) {
					final JsonObject id = (JsonObject) mapModelJsons.get(baseId);
					...
					final Iterator iterator = mr.iterator();
			...
			final PlayerItemRenderer[] var16 = (PlayerItemRenderer[]) listModels
	...
	private static ModelRenderer parseModelRenderer(final JsonObject elem, final ModelBase modelBase) {
		...
		if (var20 != null) {
			for (int var21 = 0; var21 < var20.size(); ++var21) {
				...
				final int[] var25 = Json.parseIntArray(var22.get("textureOffset"), 2);

				if (var25 == null) {
					throw new JsonParseException("Texture offset not specified");
				}

				final float[] var27 = Json.parseFloatArray(var22.get("coordinates"), 6);
		...
		if (submodel != null) {
			final ModelRenderer var23 = parseModelRenderer(submodel, modelBase);
			mr.addChild(var23);
		}

		final JsonArray var24 = (JsonArray) elem.get("submodels");

		if (var24 != null) {
			for (int var26 = 0; var26 < var24.size(); ++var26) {
				final JsonObject var28 = (JsonObject) var24.get(var26);
