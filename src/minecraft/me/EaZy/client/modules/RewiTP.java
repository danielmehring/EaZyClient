...
public class RewiTP extends Module {

	public static RewiTP mod;
	...
	public RewiTP() {
		super("RewiTP", 0, "", Category.PLAYER);
		mod = this;
	}
		...
		if (to != null) {
			if (Minecraft.thePlayer.isSneaking()) {
				...
				if (!Double.isNaN(yawToPos)) {
					...
					final double dX = -Math.sin(yawToPos) * 0.04;
					final double dZ = Math.cos(yawToPos) * 0.04;
					Minecraft.thePlayer.setPosition(Minecraft.thePlayer.posX + dX,
							Minecraft.thePlayer.posY + (to.getY() == mc.thePlayer.posY ? 0.25 : 0.125),
