...
public static final int EaZy = 2017;
	...
	private static IntBuffer nextIntBuffer(final int size) {
		final ByteBuffer buffer = bigBuffer;
	...
	public static void loadConfig() {
		...
		if (configFile.exists()) {
			try {
				final FileReader ops = new FileReader(configFile);
		...
		for (final EnumShaderOption op : var9) {
			final String key = op.getPropertyKey();
			final String def = op.getValueDefault();
			final String val = shadersConfig.getProperty(key, def);
	...
	public static void storeConfig() {
		...
		for (final EnumShaderOption op : ops) {
			final String key = op.getPropertyKey();
		...
		try {
			final FileWriter var6 = new FileWriter(configFile);
	...
	public static void loadShaderPack() {
		final boolean shaderPackLoadedPrev = shaderPackLoaded;
		...
		final String packName = shadersConfig.getProperty(EnumShaderOption.SHADER_PACK.getPropertyKey(),
				packNameDefault);
		...
		if (!packName.isEmpty() && !packName.equals(packNameNone) && !shadersBlocked) {
			if (packName.equals(packNameDefault)) {
				...
				try {
					final File formatChanged = new File(shaderpacksdir, packName);
		...
		final boolean formatChanged1 = shaderPackLoaded != shaderPackLoadedPrev;
	...
	private static void loadShaderPackProperties() {
		...
		if (shaderPack != null) {
			...
			try {
				final InputStream e = shaderPack.getResourceAsStream(path);
	...
	public static ShaderOption[] getShaderPackOptions(final String screenName) {
		...
		if (shaderPackGuiScreens == null) {
			if (shaderPackProfiles != null) {
				final ShaderOptionProfile var8 = new ShaderOptionProfile(shaderPackProfiles, ops);
			...
			final ShaderOption[] sos = shaderPackGuiScreens.get(key);
			...
			if (sos == null) {
				...
				for (final ShaderOption so : sos) {
					if (so == null) {
						list.add((Object) null);
					} else if (so instanceof ShaderOptionRest) {
						final ShaderOption[] restOps = getShaderOptionsRest(shaderPackGuiScreens, ops);
				...
				final ShaderOption[] var9 = (ShaderOption[]) list.toArray(new ShaderOption[list.size()]);
				return var9;
			}
		}
	}

	private static ShaderOption[] getShaderOptionsRest(final Map<String, ShaderOption[]> mapScreens,
			final ShaderOption[] ops) {
		final HashSet setNames = new HashSet();
		final Set keys = mapScreens.keySet();
		final Iterator list = keys.iterator();

		while (list.hasNext()) {
			final String sos = (String) list.next();
			final ShaderOption[] so = mapScreens.get(sos);

			for (final ShaderOption so1 : so) {
				if (so1 != null) {
		...
		for (final ShaderOption var12 : ops) {
			if (var12.isVisible()) {
				final String var13 = var12.getName();
		...
		final ShaderOption[] var11 = (ShaderOption[]) var9.toArray(new ShaderOption[var9.size()]);
		return var11;
	}
	...
	private static ShaderOption[] getVisibleOptions(final ShaderOption[] ops) {
		...
		for (final ShaderOption so : ops) {
			if (so.isVisible()) {
				list.add(so);
			}
		}

		final ShaderOption[] var4 = (ShaderOption[]) list.toArray(new ShaderOption[list.size()]);
		return var4;
	}
	...
	private static void saveOptionProperties(final IShaderPack sp, final Properties props) throws IOException {
		final String path = shaderpacksdirname + "/" + sp.getName() + ".txt";
	...
	private static ShaderOption[] loadShaderPackOptions() {
		try {
			final ShaderOption[] e = ShaderPackParser.parseShaderPackOptions(shaderPack, programNames,
					shaderPackDimensions);
			final Properties props = loadOptionProperties(shaderPack);

			for (final ShaderOption element : e) {
				final ShaderOption so = element;
	...
	static ArrayList listOfShaders() {
		...
		try {
			...
			final File[] e = shaderpacksdir.listFiles();

			for (final File file : e) {
			...
			if (!skipPrint) {
				if (errorCode == 1286) {
					...
					System.err.format("GL error 0x%04X: %s (Fb status 0x%04X) at %s\n",
							new Object[] { errorCode, GLU.gluErrorString(errorCode), status, location });
				} else {
					System.err.format("GL error 0x%04X: %s at %s\n",
							new Object[] { errorCode, GLU.gluErrorString(errorCode), location });
				}
			}
		}
	...
	public static int checkGLError(final String location, final String info) {
		...
		if (errorCode != 0) {
			System.err.format("GL error 0x%04x: %s at %s %s\n",
					new Object[] { errorCode, GLU.gluErrorString(errorCode), location, info });
		}
	...
	public static int checkGLError(final String location, final String info1, final String info2) {
		...
		if (errorCode != 0) {
			System.err.format("GL error 0x%04x: %s at %s %s %s\n",
					new Object[] { errorCode, GLU.gluErrorString(errorCode), location, info1, info2 });
		}
	...
	public static void init() {
		...
		if (!isShaderPackInitialized) {
			...
			updateBlockLightLevel();
			final ShaderProfile activeProfile = ShaderUtils.detectProfile(shaderPackProfiles, shaderPackOptions, false);
			String worldPrefix = "";
			int maxDrawBuffers;
			...
			for (var12 = 0; var12 < 33; ++var12) {
				...
				if (var12 == 29) {
					...
					if (n != null) {
						final IntBuffer var14 = drawBuffersBuffer[var12];
						int numDB = n.length();
	...
	public static void resetDisplayListModels() {
		if (needResetModels) {
			...
			final Iterator it = Minecraft.getRenderManager().getEntityRenderMap().values().iterator();

			while (it.hasNext()) {
				final Render ren = (Render) it.next();
	...
	public static void resetDisplayListModel(final ModelBase model) {
		if (model != null) {
			final Iterator it = model.boxList.iterator();
	...
	public static void resetDisplayListModelRenderer(final ModelRenderer mrr) {
		...
		if (mrr.childModels != null) {
			...
			for (final int n = mrr.childModels.size(); i < n; ++i) {
	...
	private static int createVertShader(final String filename) {
		...
		if (vertShader == 0) {
			...
			final ShaderOption[] activeOptions = getChangedOptions(shaderPackOptions);
							...
							if (line.matches("uniform [ _a-zA-Z0-9]+ shadow;.*")) {
								...
								if (line.matches("/\\* SHADOWRES:[0-9]+ \\*/.*")) {
									...
									if (line.matches(
										...
										e1.matches();
										name = e1.group(1);
										final String bufferindex2 = e1.group(2);
										final int bufferindex1 = getBufferIndexFromString(name);
	...
	private static void saveShader(final String filename, final String code) {
		try {
			final File e = new File(shaderpacksdir, "debug/" + filename);
	...
	public static void useProgram(int program) {
		...
		if (activeProgram != program) {
			...
			if (programsID[program] == 0) {
				...
				final IntBuffer drawBuffers = programsDrawBuffers[program];
				...
				setProgramUniform1f("far", Minecraft.gameSettings.renderDistanceChunks * 16);
				...
				setProgramUniform1i("hideGUI", Minecraft.gameSettings.hideGUI ? 1 : 0);
	...
	public static void setProgramUniform1i(final String name, final int x) {
		final int gp = programsID[activeProgram];

		if (gp != 0) {
			final int uniform = ARBShaderObjects.glGetUniformLocationARB(gp, name);
	...
	public static void setProgramUniformMatrix4ARB(final String name, final boolean transpose,
	...
	private static int getBufferIndexFromString(final String name) {
		return !name.equals("colortex0") && !name.equals("gcolor") ? !name.equals("colortex1") && !name.equals("gdepth")
	...
	private static int getTextureFormatFromString(String par) {
		...
		for (int i = 0; i < formatNames.length; ++i) {
			final String name = formatNames[i];
	...
	private static void loadEntityDataMap() {
		...
		if (mapBlockToEntityData.isEmpty()) {
			final Iterator reader = Block.blockRegistry.getKeys().iterator();

			while (reader.hasNext()) {
				final ResourceLocation e = (ResourceLocation) reader.next();
		...
		if (reader1 != null) {
			...
			try {
				while ((e1 = reader1.readLine()) != null) {
					final Matcher m1 = patternLoadEntityDataMap.matcher(e1);

					if (m1.matches()) {
						final String name1 = m1.group(1);
						final String value = m1.group(2);
						final int id = Integer.parseInt(value);
						final Block block = Block.getBlockFromName(name1);
	...
	public static void beginRender(final Minecraft minecraft, final float partialTicks, final long finishTimeNano) {
		...
		mc = minecraft;
		Minecraft.mcProfiler.startSection("init");
		...
		if (Minecraft.displayWidth != renderDisplayWidth || Minecraft.displayHeight != renderDisplayHeight) {
			resize();
		}
		...
		final float renderViewEntity = diffSystemTime * 0.01F;
		float i = (float) Math
		...
		final Entity var8 = mc.func_175606_aa();
		...
		final float temp2 = (float) Math.exp(Math.log(0.5D) * i / eyeBrightnessHalflife);
		...
		isEyeInWater = Minecraft.gameSettings.thirdPersonView == 0 && !isSleeping
				&& Minecraft.thePlayer.isInsideOfMaterial(Material.water) ? 1 : 0;
		final Vec3 var9 = Minecraft.theWorld.getSkyColor(mc.func_175606_aa(), partialTicks);
	...
	private static void checkWorldChanged(final World world) {
		if (currentWorld != world) {
			...
			if (oldWorld != null && world != null) {
				...
				final boolean dimShadersOld = shaderPackDimensions.contains(dimIdOld);
				final boolean dimShadersNew = shaderPackDimensions.contains(dimIdNew);
	...
	public static void setCameraShadow(final float partialTicks) {
		...
		final float angle = celestialAngle * -360.0F;
		final float angleInterval = shadowAngleInterval > 0.0F
				? angle % shadowAngleInterval - shadowAngleInterval * 0.5F : 0.0F;
		...
		final float y1 = (float) Math.sin(raSun);
		final float raTilt = sunPathRotation * ((float) Math.PI * 2F);
		float x2 = x1;
	...
	public static void postCelestialRotate() {
		final FloatBuffer modelView = tempMatrixDirectBuffer;
		modelView.clear();
				...
				if (programsID[21 + maskR] != 0) {
					...
					for (int i = 0; i < usedColorBuffers; ++i) {
						if (programsToggleColorTextures[21 + maskR][i]) {
							final int t0 = colorTexturesToggle[i];
							final int t1 = colorTexturesToggle[i] = 8 - t0;
							GlStateManager.setActiveTexture(33984 + colorTextureTextureImageUnit[i]);
	...
	public static void drawHorizon() {
		...
		final float farDistance = Minecraft.gameSettings.renderDistanceChunks * 16;
		final double xzq = farDistance * 0.9238D;
		final double xzp = farDistance * 0.3826D;
		final double xzn = -xzp;
		final double xzm = -xzq;
		final double top = 16.0D;
		final double bot = -cameraPositionY;
	...
	public static void readCenterDepth() {
		if (!isShadowPass && centerDepthSmoothEnabled) {
			...
			final float fadeScalar = diffSystemTime * 0.01F;
			final float fadeFactor = (float) Math.exp(Math.log(0.5D) * fadeScalar / centerDepthSmoothHalflife);
	...
	private static void loadShaderPackResources() {
		...
		if (shaderPackLoaded) {
			...
			try {
				final Iterator e = listFiles.iterator();

				while (e.hasNext()) {
					final String file = (String) e.next();
					final InputStream in = shaderPack.getResourceAsStream(file);
					...
					if (in != null) {
						...
						final Iterator itp = keys.iterator();

						while (itp.hasNext()) {
							final String key = (String) itp.next();
	...
	public static String translate(final String key, final String def) {
		final String str = shaderPackResources.get(key);
